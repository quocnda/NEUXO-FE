    ...
    >
      <PopoverTrigger asChild>
        <Button
          ...
          variant="white"
          {...btnProps}
          className={cn('justify-between px-4 text-base font-normal', {
            'border-main-100': isOpen,
          })}
        >
          <div className="text-readonly-border mr-2">{prefix ?? 'Choose date:'}</div>
          ...
          <div className="min-w-[24px]">
            ...
            <Show when={!suffix}>
              {' '}
              {isOpen ? (
                <ChevronUpIcon width={24} className="h-5 text-sm" />
              ) : (
                <ChevronDownIcon width={24} className="h-5 text-sm" />
              )}
            </Show>
          </div>
        </Button>
      </PopoverTrigger>
      <PopoverContent align={align} className="w-auto max-w-xs overflow-auto md:max-w-none">
        <div className="flex py-2">
          <div className="flex">
            <div className="flex flex-col">
              <div className="flex flex-wrap justify-between">
                <div className="flex flex-col items-center justify-start gap-2 px-3 pb-4 lg:flex-row lg:items-start lg:pb-0">
                  {showCompare && (
                ...
                <Select defaultValue={selectedPreset} onValueChange={setPreset}>
                  <SelectTrigger inputSize="mixin" className="h-8 w-full gap-5 rounded-md px-4 md:w-auto">
                    <SelectValue placeholder="Select..." className="rounded-md" />
                  </SelectTrigger>
                  <SelectContent>
                    {PRESETS.map((preset) => (
              ...
              <div>
                <Calendar
                  ...
                  disabled={(date) => {
                    ...
                    return disablePast ? dayjs(date).isBefore(currentDate, 'day') : false;
                  }}
                />
              </div>
            </div>
