...
const sheetVariants = cva(
  'fixed z-50 gap-4 bg-background px-6 shadow-xl ring-1 ring-black/5 border-transparent transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-300 data-[state=open]:duration-500',
  {
    variants: {
      side: {
        default:
...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(
  ({ side = 'right', className, fullWidth, children, ...props }, ref) => (
    <SheetPortal>
      <SheetOverlay />
      <SheetPrimitive.Content
        ref={ref}
        className={cn(sheetVariants({ side }), fullWidth && '!w-full', className)}
        {...props}
      >
        {children}
      </SheetPrimitive.Content>
