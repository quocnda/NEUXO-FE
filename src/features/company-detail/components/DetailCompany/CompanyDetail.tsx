...
const CompanyDetails = ({
  ...
  const [draftCountry, setDraftCountry] = useState<string>(data?.country || '');

  const resetDraftCountry = () => setDraftCountry(data?.country || '');

  const toggleEdit = () => {
    if (!isEditingCountry) {
      resetDraftCountry();
    }
    setIsEditingCountry((prev) => !prev);
  };
