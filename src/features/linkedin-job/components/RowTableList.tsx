...
const RowTableList = ({ indexRow, tableLength, item, refetch, validEmails, paramsQuery }: Props) => {
  ...
  const handleClick = useCallback(
    (event: React.MouseEvent<HTMLDivElement>) => {
      const url = `/matching-companies/company-detail/${item?.company_id}?page=linkedin-job`;
      if (item?.company_id) {
        localStorage.setItem(ENUM_PAGE.LINKEDIN_JOB_PAGE, paramsQuery.page.toString());
  ...
  return (
    <>
      <RowTable>
        <ItemRowTable indexRow={indexRow} tableLength={tableLength}>
          ...
          <Show when={!item?.company_id}>
            <div className="flex items-center gap-2">
              <Avatar src={item?.avatar_url ?? '/images/profile.svg'} className="h-5 w-5 rounded-full" />
              <TextBody1>{shortenName(item?.company, 30) || '-'}</TextBody1>
            </div>
          </Show>
        </ItemRowTable>
        <ItemRowTable indexRow={indexRow} tableLength={tableLength}>
          <Tooltip label={item?.job_title} hidden={!(item?.job_title && String(item?.job_title).length > 40)}>
            <div>
              <TextBody1>{shortenName(item?.job_title, 40) ?? '-'}</TextBody1>
            </div>
          </Tooltip>
        </ItemRowTable>
        <ItemRowTable indexRow={indexRow} tableLength={tableLength}>
        ...
        <ItemRowTable indexRow={indexRow} tableLength={tableLength}>
          <TextBody1>{item?.country ?? '-'}</TextBody1>
        </ItemRowTable>
        <ItemRowTable indexRow={indexRow} tableLength={tableLength}>
          <TextBody1>{item?.created_at ? moment(item.created_at).format('DD MMM, YYYY') : '-'}</TextBody1>
        </ItemRowTable>
