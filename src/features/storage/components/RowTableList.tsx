...
import { bgTrigger } from '@/features/matching-companies/utils/style';
import { useUserStore } from '@/stores';
import type { TypeItemTable } from '@/types/common.type';

import useServices from '../hooks/useServices';
import { getColorLabel } from '../utils/style';
...
const RowTableList = ({
  ...
  const handleClick = useCallback(
    (event: React.MouseEvent<HTMLDivElement>) => {
      const url = `/matching-companies/company-detail/${item?.company_id}?page=storage`;
      localStorage.setItem(ENUM_PAGE.STORAGE_PAGE, paramsQuery.page.toString());
  ...
  const renderColumnContent = useCallback(
    (column: any) => {
      switch (column?.title) {
        ...
        case 'trigger_time':
          return <TextBody1>{item?.updated_at ? moment(item.updated_at).format('DD MMM, YYYY') : '-'}</TextBody1>;
        case 'trigger':
          return (
            <div className="flex items-center gap-2">
        ...
        case 'location':
          return (
            <TextBody1>
              {Array.isArray(item.location) && item.location.length > 0 ? item.location.join(', ') : '-'}
            </TextBody1>
          );
        case 'followers':
